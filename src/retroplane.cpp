...
retroplane::retroplane(vector2D start, vector2D end,
...
QPair<vector2D *, qint32> retroplane::intersection_with_ray(ray *r) const
{
...
ray *retroplane::generate_ray(ray *r)
{
    /*
...
qreal retroplane::get_distance_to_point(vector2D p) const
{
    return common_functions::dist_to_stratch(edge_a, edge_b, p);
}

QString retroplane::who_i_am() const { return QString("RetroP"); }

void retroplane::generate_outline()
{
...
void retroplane::rot_by(qreal a)
{
    QTransform t;
...
void retroplane::move_by(qreal x, qreal y)
{
