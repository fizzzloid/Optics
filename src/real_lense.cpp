...
real_lense::real_lense(vector2D a, vector2D b,
                ...
                : edge_a(a), edge_b(b), radius1(r1), radius2(r2),
                  thick(thicknes), index_of_refr(index_of_refraction),
...
qreal real_lense::get_distance_to_point(vector2D p) const
{
    ...
    bool f1 = (d1 <= 0.0) ^ (R1 < 0.0);
    bool f2 = (d2 <= 0.0) ^ (R2 < 0.0);
...
void real_lense::rot_by(qreal a)
{
    QTransform t;
    auto center3 = (center1 + center2)/2;
...
void real_lense::move_by(qreal x, qreal y)
{
